...
public class ConsultaDAO implements CRUD {
    ...
    public List damePorAnno(int anno, int propietario_id) {
        ...
        String sql = "select * from consulta where anno=? and propietario_id=?";
        try {
    ...
    public int actualizar(Object[] o) {
        int respuesta = 0;
        String sql = "update consulta set propietario_id=?,  matricula=?, descripcion_averia=?, modelo=?, anno=?,estado=false where id_auto=?";
        try {
            conexion = new Conexion().getConexion();
            ps = conexion.prepareStatement(sql);
            ps.setObject(1, o[0]);
    ...
    public int guardar(Object[] o) {
        int respuesta = 0;
        String sql = "insert into consulta values (null, ?, ?, ?, ?, ?,false)";
        try {
            conexion = new Conexion().getConexion();
            ps = conexion.prepareStatement(sql);
            ps.setObject(1, o[0]);
    ...
    public void eliminar(int id) {
        String sql = "delete from consulta where id_auto=?";
        try {
            conexion = new Conexion().getConexion();
            ps = conexion.prepareStatement(sql);
            ps.setObject(1, id);
    ...
    public List<Consulta> dameAutoPorId(int id_propietario) {
        List<Consulta> miLista = new ArrayList<>();
        String sql = "select * from consulta where propietario_id=? order by estado asc";
        try {
            conexion = new Conexion().getConexion();
            ps = conexion.prepareStatement(sql);
    ...
    public void actualizarEstado(int id_auto) {
        String sql = "update consulta set estado=true where id_auto=?";
        try {
            conexion=new Conexion().getConexion();
            ps=conexion.prepareStatement(sql);
            ps.setObject(1, id_auto);
